...
class PartialTimeFixedStrikeLookbackOptionPricer:
    ...
    def get_price(self):
        ...
        if flag == 'call':
            a1 = x * math.exp(-r *t2) * N(d2)
            a2 = s * math.exp((b-r)*t2) * N(d1)
            a3 = s * math.exp(-r * t2) * math.sqrt(sigma, 2)/(2*b)
            b1 = -math.pow(s/x, -2*b/math.pow(sigma, 2)) * M(d1 - ((2*b*math.sqrt(t2))/sigma), -f1 + (2*b*math.sqrt(t2)/sigma), -math.sqrt(t1/t2))
            b2 = math.exp(b*t2) * math.pow(lmd, 2*b/math.pow(sigma, 2)) * M(-d1-g1, e1+g2, -math.sqrt(1 - (t1/t2)))
            c1 = s * math.exp((b-r)*t2) * M(-e1, d1, -math.sqrt((1-t1)/t2))
            c2 = x * math.exp(-r*t2) * M(-f2, d2-g1, -math.sqrt(t1/t2))
            c3 = math.exp(-b * (t2 - t1)) * (1 + math.pow(sigma, 2)/(2*b)) * s * math.exp((b-r)*t2) * N(f1) * N(-e2)
