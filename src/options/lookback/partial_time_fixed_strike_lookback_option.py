...
class PartialTimeFixedStrikeLookbackOption(OptionBase):
    ...
    def get_value(self):
        ...
        if self.flag == 'call':
            ...
            b1 = -math.pow(self.s / self.x, -2 * self.b / math.pow(self.sigma, 2)) * M(
                d1 - ((2 * self.b * math.sqrt(self.t2)) / self.sigma),
                -f1 + (2 * self.b * math.sqrt(self.t2) / self.sigma), -math.sqrt(self.t1 / self.t2))
            ...
            c3 = math.exp(-self.b * (self.t2 - self.t1)) * (
                1 + math.pow(self.sigma, 2) / (2 * self.b)) * self.s * math.exp((self.b - self.r) * self.t2) * N(
                f1) * N(-e2)
